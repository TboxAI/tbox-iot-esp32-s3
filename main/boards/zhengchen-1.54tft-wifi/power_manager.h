...
private:
    ...
    std::vector<uint16_t> adc_values_[2];
    ...
    const int kTemperatureReadInterval = 30; // 每 30 秒读取一次温度
    ...
    void CheckBatteryStatus() {
        ...
        if (new_charging_status != is_charging_) {
            ...
            ReadBatteryAdcData(is_charging_);
            return;
        }

        // 如果电池电量数据不足，则读取电池电量数据
        if (adc_values_[new_charging_status ? 0 : 1].size() < kBatteryAdcDataCount) {
            ReadBatteryAdcData(new_charging_status);
            return;
        }
    ...
    void ReadBatteryAdcData(bool is_charging) {
        // 读取 ADC 值
        int adc_value;
        ESP_ERROR_CHECK(adc_oneshot_read(adc_handle_, ADC_CHANNEL_7, &adc_value));
        
        auto &target_adc_values = adc_values_[is_charging ? 0 : 1];
        if (target_adc_values.size() > kBatteryAdcDataCount) {
            target_adc_values.erase(target_adc_values.begin());
        }
        uint32_t average_adc = 0;
        for (auto value : target_adc_values) {
            average_adc += value;
        }

        if (target_adc_values.size() > 0) {
            average_adc /= target_adc_values.size();
        }
        
        if (is_charging) {
            if (adc_value > average_adc || target_adc_values.empty()) {
                target_adc_values.push_back(adc_value); // 充电时，只增不减
            }
        } else {
            if (adc_value < average_adc || target_adc_values.empty()) {
                target_adc_values.push_back(adc_value); // 放电时，只减不增
            }
        }
        ...
        ESP_LOGI("PowerManager", "is_charging: %d, ADC value: %d average: %ld level: %ld", 
                is_charging, adc_value, average_adc, battery_level_);
    }
...
public:
    PowerManager(gpio_num_t pin) : charging_pin_(pin) {
        ...
        ESP_ERROR_CHECK(esp_timer_start_periodic(timer_handle_, /* 5 分钟检查一次 */1000000));
