...
void McpServer::AddCommonTools() {
    ...
    // 关键词：“蓝牙配对”，“易控全屋”
    AddTool("self.start_bt_pairing",
        "Start the pairing of peer bluetooth devices and grouped into an family room(living room, or bedroom...) so to be interconnected\n"
        "Use this tool for: \n"
        "1. Pairing of peer IoT devices(e.g. pair to a LED bulb and in living room group?)\n",
        PropertyList(),
        [&board](const PropertyList& properties) -> ReturnValue {
            if (board.GetIBeacon() != nullptr) {
                return board.GetIBeacon()->StartPairing() == ESP_OK;
            }
            return false;
        });

    AddTool("self.stop_bt_pairing",
        "Stop the pairing of peer bluetooth devices\n"
        "Use this tool for: \n"
        "1. Stop pairing of bluetooth devices(e.g. a colorful LED bulb)\n",
        PropertyList(),
        [&board](const PropertyList& properties) -> ReturnValue {
            if (board.GetIBeacon() != nullptr) {
                return board.GetIBeacon()->StopPairing() == ESP_OK;
            }
            return false;
        });

    AddTool("self.enter_blufi_configure",
        "Enter the bluetooth assisted WiFi configuration mode \n",
        PropertyList(),
        [&board](const PropertyList& properties) -> ReturnValue {
            ((WifiBoard&)board).ResetWifiConfiguration();
            return true;
        });
