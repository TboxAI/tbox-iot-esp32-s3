...
int64_t NowMs() {
    ...
    int64_t ms_int = milliseconds.count();
    return ms_int;
}
...
void Application::Start() {
    ...
    opus_decoder_ = std::make_unique<OpusDecoderWrapper>(codec->output_sample_rate(), 1, frame_duration_ms);
#endif
    ...
    esp_timer_start_periodic(clock_timer_handle_, 2000000);
    ...
    protocol_->OnNetworkError([this](const std::string& message) {
        SetDeviceState(kDeviceStateIdle);
        Alert(Lang::Strings::ERROR, message.c_str(), "offline", Lang::Sounds::P3_EXCLAMATION);
    });
    ...
    protocol_->OnIncomingAudio([this](AudioStreamPacket&& packet) {
        {
            ...
            bool audio_queue_notfull = (audio_decode_queue_.size() < MAX_AUDIO_PACKETS_IN_QUEUE);
	        if (is_speaking && audio_queue_notfull) {
	            audio_decode_queue_.emplace_back(std::move(packet));
	        } else {
                ESP_LOGW(TAG, "OnIncomingAudio() dropped frame_duration=%d pkt size: %u, because: is_speaking %d, audio_queue_notfull: %d", packet.frame_duration, packet.payload.size(), is_speaking, audio_queue_notfull);
            }
		}
    });
    ...
    protocol_->OnAudioChannelClosed([this, &board]() {
        ESP_LOGI(TAG, "OnAudioChannelClosed()");
    ...
    protocol_->OnIncomingJson([this, display, &board](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            // ESP_LOGI(TAG, "tts cmd, state: %s listening_mode_:%d, device_state_:%d", state->valuestring, listening_mode_, device_state_);
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                Schedule([this]() {
                    ...
                    if (device_state_ == kDeviceStateSpeaking) {
                        ESP_LOGI(TAG, "tts stop, listening_mode_:%d, device_state_:%d", 
                            listening_mode_, device_state_);
...
#endif
#if CONFIG_IOT_PROTOCOL_XIAOZHI
        } else if (strcmp(type->valuestriUnknownng, "iot") == 0) {
            ...
            if (cJSON_IsNumber(amplitude)) {
                if (board.GetIBeacon() != nullptr) {
                    ...
                    // ESP_LOGI(TAG, "amplitude: %d", amplitude->valueint % 128);
                }
            } else {
                ESP_LOGW(TAG, "amplitude type is not a number");
            }
        } else {
            ESP_LOGW(TAG, " message type: %s", type->valuestring);
            auto msg = cJSON_GetObjectItem(root, "message");
            if (cJSON_IsString(msg)) {
                ESP_LOGW(TAG, "Unknown message content: %s", msg->valuestring);
            }
        }
    });
        ...
        background_task_->Schedule([this, data = std::move(data)]() mutable {
            opus_encoder_->Encode(std::move(data), [this](std::vector<uint8_t>&& opus) {
                ...
                if (!protocol_->SendAudio(packet)) {
                    ESP_LOGE(TAG, "Send packet failed, case 1, size: %u", packet.payload.size());
                }
            });
        });
    });
    ...
    audio_processor_->OnVadStateChange([this](bool speaking) {
        if (device_state_ == kDeviceStateListening) {
            Schedule([this, speaking]() {
                ESP_LOGI(TAG, "OnVadStateChange() speaking: %d", (int)speaking);
                if (speaking) {
    ...
    // UBaseType_t initial_stack_hwm = uxTaskGetStackHighWaterMark(NULL);
    // ESP_LOGE(TAG, "Main Task started. Initial Stack HWM: %u words (%u bytes)", 
    //          initial_stack_hwm, initial_stack_hwm * 4);
...
void Application::OnClockTimer() {
    ...
    // Print the debug info every 4 seconds
    // if (clock_ticks_ % 2 == 0) {
    ...
    if (ota_.HasServerTime()) {
        if (device_state_ == kDeviceStateIdle) {
            bool display_text_status = clock_ticks_ % 2 == 0;
            Schedule([this, display_text_status]() {
                if (display_text_status) {
...
#ifdef CONFIG_USE_AUDIO_CODEC_DECODE_OPUS
        WriteAudio(packet.payload, packet.sample_rate);
#else
        std::vector<int16_t> pcm;
        // auto start = esp_timer_get_time(); // Reset the timer to avoid blocking the audio output
        if (!opus_decoder_->Decode(std::move(packet.payload), pcm)) {
            return;
        }
        // auto decode_elapsed = esp_timer_get_time() - start;
        // start = esp_timer_get_time();
        WriteAudio(pcm, opus_decoder_->sample_rate());
        // auto write_audio_elapsed = esp_timer_get_time() - start;
        // ESP_LOGE(TAG, "Decode audio packet takes %d ms, write takes: %d ms, size: %d", 
        //     (int)decode_elapsed/1000, (int)write_audio_elapsed/1000, (int)packet.payload.size());
#endif
#ifdef CONFIG_USE_SERVER_AEC
        std::lock_guard<std::mutex> lock(timestamp_mutex_);
...
#endif
        if (!protocol_->SendAudio(packet)) {
            ESP_LOGE(TAG, "Send packet failed, case 2, size: %u", packet.payload.size());
        }
#else
        std::vector<int16_t> data;
