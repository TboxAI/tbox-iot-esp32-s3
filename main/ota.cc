...
Http* Ota::SetupHttp() {
    ...
    http->SetHeader("Device-Key", device_key.c_str());
    http->SetHeader("Client-Id", board.GetUuid());
    http->SetHeader("x-webgw-appId", "180020010001263215");
    http->SetHeader("x-webgw-version", "2.0");
...
bool Ota::CheckVersion() {
    ...
    ESP_LOGD(TAG, "Current version: %s, ota_url: %s", current_version_.c_str(), url.c_str());
    if (url.length() < 10) {
    ...
    if (cJSON_IsObject(websocket)) {
        ...
        cJSON_ArrayForEach(item, websocket) {
            if (cJSON_IsString(item)) {
                ESP_LOGI(TAG, "写入 websocket 配置(strig)：" "%s = %s", item->string, item->valuestring);
                settings.SetString(item->string, item->valuestring);
            } else if (cJSON_IsNumber(item)) {
                ESP_LOGI(TAG, "写入 websocket 配置(number)：" "%s = %d", item->string, item->valueint);
